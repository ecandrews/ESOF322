...
public class BasicPropTest {
	...
	@Test
	public void basicPropConstructorTest() {
		int[] rentList = {1,1,1,1,1};
		BasicProp testProp = new BasicProp (rentList, "brown", 100,4,rentList);
		assertTrue(testProp.getColor() == "brown" );
		assertTrue(testProp.getValue() == 100);
		assertTrue(testProp.getBuildingCost() == 50);
		assertTrue(testProp.getLocation() == 4);
		assertTrue(testProp.getBuildings() == 0);
	}
	  @Test
	  public void landedOnTest(){
	  	int[] rentList = {1,1,1,1,1};
	  	Player tester = new Player("Bud");
	  	BasicProp test = new BasicProp(rentList, "color", 3,3,rentList);
	  	assertEquals(0,test.landedOn(tester,2));
	  	test.setOwner(tester);
	  	assertEquals(1,test.landedOn(tester,2));
	  }
