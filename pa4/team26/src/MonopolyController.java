...
public class MonopolyController implements Initializable{
    ...
    public void takeTurn(Player player)
    {
        ...
        mysteryMeatbtn.setDisable(true);
        this.turnOver = false;
    ...
    public void movePlayer() {
        ...
        if(nowPlayer.getInJail()){
            ...
            if(this.numDubs == 3){
                ...
                if(this.nowPlayer.isMysteryMeat())
                {
                    mysteryMeatbtn.setDisable(false);
                }
    ...
    Random randNumUtil;
    public void mysteryMeat()
    {
        mysteryMeatbtn.setOnAction(e -> {
            int[] result = MysteryPopup.display();
            // 1 = Card 1 Advance to Go
            if (result[0] == 1){
                this.nowPlayer.setLocationOnBoard(0);
                mysteryMeatbtn.setDisable(true);
            }
            // 2 = Card 2 Bank Error, collect $200
            else if (result[0] == 2){
                nowPlayer.setCashOnHand(200);
                mysteryMeatbtn.setDisable(true);
            }
            //3 = Card 3 Doctor's fees, pay $50
            else if (result[0] == 3){
                nowPlayer.setCashOnHand(-50);
                mysteryMeatbtn.setDisable(true);
            }
            //4 Card 4 From sale of Hamburger Helper stock you get $50
            else if (result[0] == 4){
                nowPlayer.setCashOnHand(50);
                mysteryMeatbtn.setDisable(true);
            }
            //5 = Card 5 Get out of jail free card
            else if (result[0] == 5){
            ...
            //7 = Card 7 Grand opera night collect $50 from every player
            else if (result[0] == 7){
                for(int i = 0; i < this.model.allPlayers.size(); i++)
                {
                    (this.model.allPlayers.get(i)).setCashOnHand(-50);
                    nowPlayer.setCashOnHand(50);
                }
                mysteryMeatbtn.setDisable(true);
            }
            //8 = Card 8 holiday fund matures
            else if (result[0] == 8){
                nowPlayer.setCashOnHand(100);
                mysteryMeatbtn.setDisable(true);
            }
            //9 = Card 9 income tax refund collect $20
            else if (result[0] == 9){
                nowPlayer.setCashOnHand(20);
                mysteryMeatbtn.setDisable(true);
            }
            //10 = Card 10 birthday, collect $10 from every player
            else if (result[0] == 10){
                for(int i = 0; i < this.model.allPlayers.size(); i++)
                {
                    (this.model.allPlayers.get(i)).setCashOnHand(-10);
                    nowPlayer.setCashOnHand(10);
                }
                mysteryMeatbtn.setDisable(true);
            }
            //11 = Card 11 Life insurance matures collect $100
            else if (result[0] == 11){
                nowPlayer.setCashOnHand(100);
                mysteryMeatbtn.setDisable(true);
            }
            //12 = Card 12 Pay hospital fees of $100
            else if (result[0] == 12){
                nowPlayer.setCashOnHand(-100);
                mysteryMeatbtn.setDisable(true);
            }
            //13 = Card 13 Pay health inspector fees of $150
            else if (result[0] == 13){
                nowPlayer.setCashOnHand(-150);
                mysteryMeatbtn.setDisable(true);
            }
            //14 = Card 14 Receive $25 consultancy fee
            else if (result[0] == 14){
                nowPlayer.setCashOnHand(25);
                mysteryMeatbtn.setDisable(true);
            }
            //15 = Card 15 You have won second prize in a hamburger flipping contest collect $10
            else if (result[0] == 15){
                nowPlayer.setCashOnHand(10);
                mysteryMeatbtn.setDisable(true);
            }
            //16 = Card 16 You inherit $100
            else if (result[0] == 16){
                nowPlayer.setCashOnHand(100);
                mysteryMeatbtn.setDisable(true);
            }
...
//
        iceChestbtn.setOnAction(e -> {
            ...
            //10 = Card 10 Make general repairs on all your property – For each house pay $25 – For each hotel $100
                ...
                ArrayList props = nowPlayer.getPropertiesOwned();
                for(int i=0; i < props.size(); i++)
                {
                    ...
                    if(p.getBuildings() == 5)
                    {
                        ...
                        nowPlayer.setCashOnHand((p.getBuildings()) * 25 * (-1));
                    }
                }
                iceChestbtn.setDisable(true);
            }
